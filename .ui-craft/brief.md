...
- **`#heroBpm` stays bold (700), not semibold.** — 2026-09-05, corrected after a `/typeset` pass: a `/typeset` run dropped the hero BPM number from bold to semibold, reasoning that display type over ~600 weight generally "reads cheap." The user reverted it — the heavier weight pulls in more attention on the one number that matters most while playing, which serves principle 1 ("the glance beats the click") more directly than the generic type-weight guideline does. Future `/typeset`/`/polish` passes should leave `#heroBpm` at `--font-weight-bold`.
